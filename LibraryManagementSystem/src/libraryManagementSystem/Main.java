...
public class Main {
	...
	public static void login() {
		JFrame f = new JFrame("Login");// creating instance
		JLabel l1, l2;
		l1 = new JLabel("Username"); // Create Label user
		l1.setBounds(30, 15, 100, 30);

		l2 = new JLabel("Password"); // Create label Pass
		...
		login_but.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				String username = F_user.getText(); // Store username entered bt the user in the variable "username"
				...
				} else if (password.equals(" ")) // If password is null
				{
				...
				} else { // If both the field are present then
					...
					try {
						...
						String st = ("SELECT * FROM USERS WHERE USERNAME = '" + username + "' AND PASSWORD= '"
								+ password + "'"); // retrive username and password from users
						ResultSet rs = stmt.executeQuery(st); // Execute query
						if (rs.next() == false) { // Move pointer below
							System.out.print("No user");
							JOptionPane.showMessageDialog(null, "Wrong Username/Password!");// DisplayMassage
						} else {
							f.dispose();
							rs.beforeFirst(); // move the pointer
							while (rs.next()) {
								String admin = rs.getString("ADMIN"); // user is admin
								System.out.println(admin);
								String UID = rs.getString("UID"); // get user id pf the user
								if (admin.equals("1")) {
									admin_menu(); // redirect ot admin menu
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (databaseName.equalsIgnoreCase("LIBRARY")) {
					...
					System.out.println("yes");
					// Drop database if it pre-exits to reset the complete database
			...
			Statement stmt = connection.createStatement();
			String sql = "CREATE DATABASE LIBRARY"; // CREARE DATABASE
			stmt.executeUpdate(sql);
		...
		view_but.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					/*
					 * ResultSetMetaData rsmd = rs.getMetaData(); DefaultTableModel model =
					 * (DefaultTableModel) book_list.getModel();
					 * 
					 * int colm = rsmd.getColumnCount(); String colName[] = new String[colm];
					 * for(int i=0;i<colm;i++) colName[i] = rsmd.getColumnName(i+1);
					 * model.setColumnIdentifiers(colName);
					 */

					book_list.setModel(resultSetToTableModel(rs));
		...
		view_but.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					f.setVisible(true);
					f.setLocationRelativeTo(null);
					// f.setDefaultCloseOperation(j.EXIT_ON_CLOSE);
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(null, e1);
				}
